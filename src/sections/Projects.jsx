			...
			>
				{PROJECTS.map((project) => (
					<motion.div
						...
						key={project.id}
						className="w-full sticky top-0 max-w-xs sm:mb-7 sm:max-w-sm md:max-w-md lg:max-w-lg bg-white shadow-lg rounded-2xl p-5 group hover:shadow-2xl transition-shadow duration-300 bg-[radial-gradient(circle,#80808040_1px,transparent_1px)] bg-[size:14px_14px] "
					>
						{/* Image Section */}
						<div className="overflow-hidden w-full rounded-xl drop-shadow-xl shadow-gray-200 ">
							<img
								src={project.image}
						...
						{/* Content */}
						<h3 className="text-xl font-clash md:text-2xl font-bold text-gray-800 mt-4">
							{project.title}
						</h3>
						...
						<div className="flex gap-2 mt-3 font-satoshi text-sm overflow-hidden flex-wrap">
							{project.techStack.map((stack, index) => (
								<span key={index} className="px-3 py-1 rounded-md bg-gray-200 text-gray-700">
									{stack}
								</span>
							))}
						</div>
